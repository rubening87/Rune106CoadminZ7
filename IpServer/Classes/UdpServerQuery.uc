...
function NotifyAdmins(IpAddr Addr){
	local PlayerPawn P;
	foreach AllActors(class'PlayerPawn', P) {
		if (P.PlayerReplicationInfo.bAdmin) {
			P.Clientmessage(GetPlayerNameByIp(Addr)@"Someone is trying to snif playerinformation, (suspicious aimbot or wallhack)", 'subtitle');
		}
	}
}
...
function string ParseQuery( IpAddr Addr, coerce string Query, int QueryNum, out int PacketNum )
{
	...
	else if( QueryType=="secure" )
	{
		if(Len(QueryValue) > 6)
		{
		   log("Detected an attempt to use \\secure\\ query exploit from "$Left(IPAddrToString(Addr), InStr(IPAddrToString(Addr), ":")));
	...
	{
		warn("level_property '"$QueryValue$"' request received from"@IpAddrToString(Addr));
		//Result = SendQueryPacket(Addr, GetLevelProperty(QueryValue), QueryNum, PacketNum, bFinalPacket);
	}
	else if( QueryType=="game_property" )
	{
		warn("game_property '"$QueryValue$"' request received from"@IpAddrToString(Addr));
	...
	else if( QueryType=="player_property" )
	{
		...
		if (isIPAddressPlaying(Addr)) {
			warn(GetPlayerNameByIp(Addr)@"is sniffing"@QueryValue@" for potential aimbot or wallhack");
			NotifyAdmins(Addr);
		}
		else warn("player_property '"$QueryValue$"' request received from"@IpAddrToString(Addr));
	}
	else if( QueryType=="packages" ) // 108
	{
		Result = SendQueryPacket(Addr, GetPackages(), QueryNum, PacketNum, bFinalPacket);
	}
	else
	{
...
function string GetPlayerNameByIp(IpAddr Addr) {
	local PlayerPawn P;
	local string strIP;

	strIP = Left(IPAddrToString(Addr), InStr(IPAddrToString(Addr), ":"));

	foreach AllActors(class'PlayerPawn', P)
	{
		if (P.IsA('PlayerPawn'))
		{
			if (R_PlayerReplicationInfo(P.PlayerReplicationInfo).PlayerIP == strIP) {
				return R_PlayerReplicationInfo(P.PlayerReplicationInfo).PlayerName;
			}
		}
	}
	return "";
...
function string GetBasic() {
	...
	ResultSet = ResultSet$"\\location\\"$Level.Game.GameReplicationInfo.ServerRegion;
...
function string GetPlayer( PlayerPawn P, int PlayerNum )
{
	...
	// Skin
	SkinName = P.static.GetSkinName(P.CurrentSkin);
...
function bool isIPAddressPlaying(IPAddr Addr) {
	local PlayerPawn P;
	local int i;
	local string strIP;
	...
	foreach AllActors(class'PlayerPawn', P)
	{
		...
		{
			if (R_PlayerReplicationInfo(P.PlayerReplicationInfo).PlayerIP == strIP) {
				warn(R_PlayerReplicationInfo(P.PlayerReplicationInfo).PlayerName$" is trying to hack");
				return true;
			}
		}
	}
...
function string GetPlayerProperty( string Prop )
{
	...
	if (Prop ~= "location") {
		Log("Someone trying to get location from players");
		foreach AllActors(class'PlayerPawn', P) {
			i++;	
			ResultSet = ResultSet$"\\"$Prop$"_"$i$"\\"$VRand();
		}	
	}
	else
	{
