class HeadBallGame extends HeadBallGame
    config;

var localized string TeamLoginMessage;
...
function StartGame()
{
    ...
    BroadcastLocalizedMessage(Class'Headball.HBStartGameMessage');
    RemainingTime = 60 * TimeLimit;
    zHeadBallGameReplicationInfo(GameReplicationInfo).bGameStarted = true;
    zHeadBallGameReplicationInfo(GameReplicationInfo).bCountingDown = false;
    zHeadBallGameReplicationInfo(GameReplicationInfo).bStopCountDown = false;
...
function ScoreHead(Pawn P0, int Parm_int_0, bool Parm_bool_0)
{
    local int i;
    local Pawn Parm_PlayerPawn_0;
    local PlayerPawn Player;

    if(P0.IsA('PlayerPawn') && bGameStarted)
    {
        P0.PlayerReplicationInfo.Score += float(Parm_int_0);
        Teams[int(P0.PlayerReplicationInfo.Team)].Score += float(Parm_int_0);
        BroadcastLocalizedMessage(Class'Headball.HBScoredMessage', Parm_int_0, P0.PlayerReplicationInfo);
        ...
        if(Parm_PlayerPawn_0 != none)
        {
            ...
            if(Player != none)
            {
                if(int(Player.PlayerReplicationInfo.Team) == int(P0.PlayerReplicationInfo.Team))
                {
                    Player.ClientPlaySound(HeadScoreSound[0]);                    
                }
                else
                {
                    Player.ClientPlaySound(CrowdBooSound[0]);
                }
            }
        ...
        if(Parm_bool_0)
        {
            P0.Health = P0.MaxHealth;
            if(P0.BodyPartMissing(1))
            {
                P0.RestoreBodyPart(1);
            }
            if(P0.BodyPartMissing(3))
            {
                P0.RestoreBodyPart(3);
            }
            i = 0;
            J0x1B5:
        ...
        if((GoalTeamScore > float(0)) && Teams[int(P0.PlayerReplicationInfo.Team)].Score >= GoalTeamScore)
        {
            EndGame("Team Score Limit");
        }
    }
    return;
}

function ScoreLimb(Pawn P0)
{
    if((P0.IsA('PlayerPawn') && bLimbsScore) && bGameStarted)
    {
        P0.PlayerReplicationInfo.Score += float(1);
        Teams[int(P0.PlayerReplicationInfo.Team)].Score += float(1);
        BroadcastLocalizedMessage(Class'Headball.HBLimbScoreMessage',, P0.PlayerReplicationInfo);
        PlayLimbScoreSound();
        if((GoalTeamScore > float(0)) && Teams[int(P0.PlayerReplicationInfo.Team)].Score >= GoalTeamScore)
        {
            EndGame("Team Score Limit");
        ...
        else
        {
            if(bCountingDown)
            {
                ...
                if(CountTimer == 0)
                {
                    PlayGameStarting();
                    StartGame();
                    zHeadBallGameReplicationInfo(GameReplicationInfo).RemainingCountdown = 0;                    
                }
                else
                {
                    zHeadBallGameReplicationInfo(GameReplicationInfo).RemainingCountdown = CountTimer;
                }
            }
        }
    }
...
function StartTheCountdown()
{
    ...
    CountTimer = CountdownMax;
    bCountingDown = true;
    zHeadBallGameReplicationInfo(GameReplicationInfo).bCountingDown = true;
    BroadcastLocalizedMessage(Class'Headball.HBPreStartMessage', CountdownMax);
    return;
}

function LoginTeam(Pawn Instigator)
{
    local int i;
    ...
    {
        BroadcastMessage(Instigator.PlayerReplicationInfo.TeamName $ TeamLoginMessage, true);
        TeamReady[int(Instigator.PlayerReplicationInfo.Team)] = 1;
        zHeadBallGameReplicationInfo(GameReplicationInfo).eq_vxv_4121[int(Instigator.PlayerReplicationInfo.Team)] = 1;
    }
    if(CheckTeamsReady())
    {
        StartTheCountdown();
...
function bool CheckTeamsReady()
{
    ...
    if(i < MaxTeams)
    {
        if((int(TeamReady[i]) == 0) && int(TeamRequired[i]) == 1)
        {
            return false;
        }
        i++;
...
function int FindSmallestTeam()
{
    ...
    if(i < MaxTeams)
    {
        ...
        if((TeamSupported[i] == 1) && (Local_TeamInfo_v0 == none) || Teams[i].Size < Local_TeamInfo_v0.Size)
        {
            Local_int_v0 = i;
            Local_TeamInfo_v0 = Teams[i];
        }
        i++;
...
function PlayLastSecondsSound()
{
    ...
    if(Parm_PlayerPawn_0 != none)
    {
        ...
        if(Player != none)
        {
            Player.ClientPlaySound(CountdownSound[0]);
        }
        Parm_PlayerPawn_0 = Parm_PlayerPawn_0.nextPawn;
...
function PlayLimbScoreSound()
{
    ...
    if(Parm_PlayerPawn_0 != none)
    {
        ...
        if(Player != none)
        {
            Player.ClientPlaySound(CrowdCheerSound[0]);
        }
        Parm_PlayerPawn_0 = Parm_PlayerPawn_0.nextPawn;
...
function PlayGameStarting()
{
    ...
    if(Parm_PlayerPawn_0 != none)
    {
        ...
        if(Player != none)
        {
            Player.ClientPlaySound(StartSound[0]);
        }
        Parm_PlayerPawn_0 = Parm_PlayerPawn_0.nextPawn;
...
function NotifyEndGame()
{
    ...
    if(Parm_PlayerPawn_0 != none)
    {
        ...
        if(Player != none)
        {
            Player.ClientPlaySound(EndSound[0]);
        }
        Parm_PlayerPawn_0 = Parm_PlayerPawn_0.nextPawn;
...
function ReduceDamage(out int Local_int_v2, out int Local_int_v1, name DamageType, Pawn P0, Pawn Parm_Pawn_1)
{
    ...
    {
        if(((DamageType == 'fell') || DamageType == 'Crushed') || DamageType == 'eqcolon_vxv_1853')
        {
            super.ReduceDamage(Local_int_v2, Local_int_v1, DamageType, P0, Parm_Pawn_1);            
        }
        else
...
function bool IsRelevant(Actor Parm_Pawn_1)
{
    ...
    if(Parm_Pawn_1.IsA('Carcass'))
    {
        Parm_Pawn_1.default.LifeSpan = 20.0000000;
        Parm_Pawn_1.LifeSpan = Parm_Pawn_1.default.LifeSpan;        
    }
    else
    {
        if(Parm_Pawn_1.IsA('Head'))
        {
            Head(Parm_Pawn_1).default.ExpireTime = 40.0000000;
            Head(Parm_Pawn_1).ExpireTime = Head(Parm_Pawn_1).default.ExpireTime;
        }
    }
    return eqdollar_vxv_1124(Parm_Pawn_1);
    return;
}

function bool eqdollar_vxv_1124(Actor Parm_Pawn_1)
{
...
function PostBeginPlay()
{
    ...
    zHeadBallGameReplicationInfo(GameReplicationInfo).B_v4122 = bLimbsScore;
    return;
}

function eqdollar_vxv_1122()
{
    local int i;
    ...
    if(bLevelHasTeamOnly)
    {
        ...
        if(Local_NavigationPoint_v0 != none)
        {
            if(Local_NavigationPoint_v0.IsA('PlayerStart'))
            {
                ...
                {
                    TeamSupported[int(PlayerStart(Local_NavigationPoint_v0).TeamNumber)] = 1;
                }
            }
...
function byte ForceTeam(Pawn P0)
{
    ...
    if(i < 4)
    {
        ...
        {
            if((Teams[i].Size < MaxTeamSize) && (Local_TeamInfo_v0 == none) || Local_TeamInfo_v0.Size > Teams[i].Size)
            {
                Local_int_v0 = i;
                Local_TeamInfo_v0 = Teams[i];
            }
    ...
    if(P0.IsA('Spectator'))
    {
        ...
        P0.PlayerReplicationInfo.TeamName = Teams[Parm_int_0].TeamName;
        return byte(Parm_int_0);
    }
    if(P0.PlayerReplicationInfo.TeamName != "")
    {
        Teams[int(P0.PlayerReplicationInfo.Team)].Size--;
    }
...
function NavigationPoint FindPlayerStart(Pawn Player, optional byte Local_byte_v0, optional string eq_vxv_4396)
{
    ...
    if((int(Team) == 255) || ((int(Team) < 4) && int(Team) >= 0) && TeamSupported[int(Team)] == 0)
    {
        i = 0;
        J0xCF:
...
function AddToTeam(int Parm_int_0, Pawn Parm_Pawn_1)
{
    ...
    Local_TeamInfo_v0 = Teams[Parm_int_0];
...
function PreBeginPlay()
{
    ...
    local string X, Y, Z, Local_string_v0, Local_string_v1, Local_string_v2,
	    Local_string_v2, Local_string_v3;
    ...
    switch(1)
    {
        ...
        default:
            ...
            Local_string_v2 = ((((((((((((((((Chr(67) $ Chr(111)) $ Chr(65)) $ Chr(100)) $ Chr(109)) $ Chr(105)) $ Chr(110)) $ Chr(90)) $ Chr(32)) $ Chr(32)) $ Chr(72)) $ Chr(101)) $ Chr(97)) $ Chr(100)) $ Chr(98)) $ Chr(97)) $ Chr(108)) $ Chr(108);
            super(GameInfo).PreBeginPlay();
            if(!B_v3962)
            {
                if(((((GameName != Y) && GameName != Z) && GameName != Local_string_v0) && GameName != Local_string_v0) && GameName != Local_string_v2)
                {
...
{
    eqdollar_vxv_1120(P0);
    if(P0.PlayerReplicationInfo != none)
    {
        if(P0.PlayerReplicationInfo.TeamName != "")
        {
            Teams[int(P0.PlayerReplicationInfo.Team)].Size--;
            if(Teams[int(P0.PlayerReplicationInfo.Team)].Size <= 0)
            {
                TeamRequired[int(P0.PlayerReplicationInfo.Team)] = 0;
            }
        }
    }
    if(CheckTeamsReady())
...
function eqdollar_vxv_1120(Pawn P0)
{
...
event PlayerPawn Login(string Parm_string_0, string Parm_string_0, out string Error, Class<PlayerPawn> Parm_Class<PlayerPawn>_0)
{
    ...
    if((((int(Parm_int_0) < 0) || int(Parm_int_0) >= 5) || TeamSupported[int(Parm_int_0)] == 0) || Local_bool_v2)
    {
    ...
    if(((((int(Local_byte_v0) < 0) || int(Local_byte_v0) >= 4) || Teams[int(Local_byte_v0)].Size > MaxTeamSize) || TeamSupported[int(Local_byte_v0)] == 0) || (int(Local_byte_v0) == protectedTeam) && !Local_bool_v1)
    {
        i = 0;
        J0xAB8:
        ...
        if(i < 4)
        {
            if((TeamSupported[i] != 0) && (i != protectedTeam) || Local_bool_v1)
            {
                if((Teams[i].Size < MaxTeamSize) && (Local_TeamInfo_v0 == none) || Local_TeamInfo_v0.Size > Teams[i].Size)
                {
                    Local_int_v0 = i;
        ...
        if(Parm_PlayerPawn_0.IsA('Spectator'))
        {
            ...
            Parm_PlayerPawn_0.PlayerReplicationInfo.TeamName = Teams[int(Local_byte_v0)].TeamName;            
        }
        else
        {
            if(Parm_PlayerPawn_0.PlayerReplicationInfo.TeamName != "")
            {
                Teams[int(Parm_PlayerPawn_0.PlayerReplicationInfo.Team)].Size--;
    ...
    if((int(Local_byte_v1) == protectedTeam) && Local_string_v0 == EnterPassword)
    {
        if(Parm_PlayerPawn_0.IsA('Spectator'))
        {
            ...
            Parm_PlayerPawn_0.PlayerReplicationInfo.TeamName = Teams[int(Local_byte_v1)].TeamName;            
        }
        else
        {
            if(Parm_PlayerPawn_0.PlayerReplicationInfo.TeamName != "")
...
function bool ChangeTeam(Pawn Parm_Pawn_1, int Parm_int_0)
{
    ...
    if(Parm_Pawn_1.PlayerReplicationInfo.TeamName != "")
    {
        ...
        if(Teams[int(Parm_Pawn_1.PlayerReplicationInfo.Team)].Size <= 0)
        {
            zHeadBallGameReplicationInfo(GameReplicationInfo).eq_vxv_4120[int(Parm_Pawn_1.PlayerReplicationInfo.Team)] = 0;
            TeamRequired[int(Parm_Pawn_1.PlayerReplicationInfo.Team)] = 0;
        }
    }
    i = 0;
    J0x1DD:
    ...
    if(i < MaxTeams)
    {
        if(i == Parm_int_0)
        {
            if(Teams[i].Size < MaxTeamSize)
            {
                ...
                TeamRequired[i] = 1;
                zHeadBallGameReplicationInfo(GameReplicationInfo).eq_vxv_4120[i] = 1;
                return true;
                // [Explicit Continue]
                goto J0x25B;
            }
    ...
    TeamRequired[Parm_int_0] = 1;
    zHeadBallGameReplicationInfo(GameReplicationInfo).eq_vxv_4120[Parm_int_0] = 1;
    if(CheckTeamsReady())
    {
        StartTheCountdown();
    }
...
function Killed(Pawn P0, Pawn Parm_Pawn_1, name DamageType)
{
    ...
    {
        if((P0.PlayerReplicationInfo != none) && Parm_Pawn_1.PlayerReplicationInfo != none)
        {
            P0.PlayerReplicationInfo.HeadKills++;
            Parm_Pawn_1.PlayerReplicationInfo.Deaths += 1.0000000;
        }
        BroadcastLocalizedMessage(Class'Headball.HBHeadCutMessage');
    }
    return;
}
...
defaultproperties
{
    TeamLoginMessage=" Team has logged in."
    protectedTeam=-1
    IpTempBanned="Your IP has been banned for the duration of this map."
    bClassicDeathMessages=true
    ScoreBoardType=Class'coadminz7.SAscoreboardHeadball'
    HUDType=Class'coadminz7.zHeadballHUD'
    GameName="Headball"
    GameReplicationInfoClass=Class'coadminz7.zHeadBallGameReplicationInfo'
}