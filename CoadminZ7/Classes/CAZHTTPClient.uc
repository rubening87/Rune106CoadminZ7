class CAZHTTPClient extends UBrowserHTTPClient
    transient
    config;
...
function HTTPReceivedData(string Parm_string_0)
{
    if(G_ZStrg1 != none)
    {
        G_ZStrg1.CheckForUpdates(Parm_string_0, false);
    }
    return;
}
...
function HTTPError(int Parm_int_0)
{
    if(G_ZStrg1 != none)
    {
        G_ZStrg1.CheckForUpdates(string(Parm_int_0), true);
    }
    return;
}
    ...
    switch(CurrentState)
    {
        case 1:
            ...
            if(Local_bool_v0)
            {
                ...
                G_ZStrg1.CheckForUpdates(Local_string_v0, true);
                NextState = 2;
            }
            break;
        case 2:
