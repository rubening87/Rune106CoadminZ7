...
function IgnitePlayer(Pawn EventInstigator, int joint)
{
	local PawnFire F;
...
function SetCrouchHeight()
{
	...
	log("CrouchHeight: " $ CrouchHeight $ " Role: " $ Role);
}
...
function useViewShake(float DeltaTime)
{
	if(shaketimer > 0.0)
	{	
...
function PowerupFire(Pawn EventInstigator)
{
	...
	for (i=0; i<NumJoints(); i++)
	{
		if ((JointFlags[i] & JOINT_FLAG_COLLISION)!=0)
			IgnitePlayer(EventInstigator, i);
	}
}
